...
- [Canal WhatsApp](./apps/gateway-whatsapp/CONTEXT.md) — Contato (Boas-vindas, Pedido de Aceite, Aceite, Termos de Uso), Conversa, Mensagem, Mídia, Canal, Número do Canal, Janela, Template
...
Cadeia Lista → Coleta → Resposta: [`docs/arquitetura-agentes-lista-coleta.md`](./docs/arquitetura-agentes-lista-coleta.md). Testar Mercado (scan encarte / scraping site, sem Extrator): ADR [0015](./docs/adr/0015-testar-mercado-encarte-ou-site.md). Cadeia do Agente: ADR [0016](./docs/adr/0016-agente-lista-coleta-encarte-resposta.md). Canal via Cloud API: ADR [0020](./docs/adr/0020-gateway-whatsapp-cloud-api.md). Operador no backoffice: ADR [0018](./docs/adr/0018-operador-backoffice.md). Aceite no Contato no lugar da Allowlist como portão do produto: ADR [0019](./docs/adr/0019-aceite-no-contato-em-vez-de-allowlist.md).

## Relationships

- **Catálogo ↛ Canal**: o gateway não lê nem escreve Oferta; estado de envio não entra nas tabelas do catálogo (workspace ADR 0006 / 0007 / 0020)
...
- **Canal → backoffice**: o SPA lê Conversa/Mensagem (e envia texto na Allowlist) e o estado do Canal (pronto ou não configurado, Número do Canal). Tudo via `gateway-whatsapp` (ADR 0009 / 0018 / 0020), não via `ofertas-api`
- **Canal → Agente da Lista**: Mensagem viva de texto cujo Contato remetente tem Aceite entra no Agente; só é Lista se a Intenção for Lista. Sem Aceite o Canal envia Boas-vindas ou Pedido de Aceite e não chama o Agente. O gateway não chama Resposta nem o scraper (ADR 0019)
...
- **Agente da Lista → Agente de Resposta**: só depois de todas as Coletas e leituras de encarte da Lista; o organizador não busca. No mesmo Produto e Mercado, Coleta do dia prevalece; encarte preenche o resto. Relacionados saem; Marca no Termo restringe; um Produto por Item (menos extras); empate de preço lista
- **Agente → Canal**: Resposta, e na direta Consulta e Recusa, saem por `POST /envios`. Ack não dispara nessas Mensagens quando a cadeia está ligada

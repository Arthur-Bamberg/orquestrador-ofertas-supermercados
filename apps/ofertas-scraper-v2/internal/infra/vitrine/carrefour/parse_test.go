...
func TestSearchURL_LojaPadraoDoSite(t *testing.T) {
	got := carrefour.SearchURL("tomate", 0, 15)
	if strings.Contains(strings.ToLower(got), "canoas") {
		t.Fatalf("Canoas is not the site default: %s", got)
	}
	if !strings.HasPrefix(got, "https://carrefourbrfood.vtexcommercestable.com.br/api/io/_v/api/intelligent-search/product_search/trade-policy/2?") {
		t.Fatalf("got %s", got)
	}
	if !strings.Contains(got, "query=tomate") || !strings.Contains(got, "count=15") || !strings.Contains(got, "page=1") {
		t.Fatalf("got %s", got)
	}
...
func TestParseSearch_LePrecoMarcaEIndicacao(t *testing.T) {
	pagina, err := carrefour.ParseSearch(readFixture(t, "search.json"))
	if err != nil {
		t.Fatal(err)
	}
	if pagina.Total != 13 {
	...
	if pagina.HasNext {
		t.Fatal("13 cards fit in page size 15; must not HasNext")
	}
	got := pagina.Cartoes
	if len(got) != 3 {
		t.Fatalf("got %d", len(got))
	}
	if got[0].Nome != "Tomate Italiano Carrefour Aprox. 200g" || got[0].Marca != "Carrefour" {
		t.Fatalf("%#v", got[0])
	}
	if got[0].Valor != 13.99 {
		t.Fatalf("KG shelf is per-kg 13.99, got %v", got[0].Valor)
	}
	...
	if got[1].Nome != "Tomate Grape Ferrara 180 g" || got[1].Valor != 5.99 || got[1].IndicacaoPromocional {
		t.Fatalf("regular card %#v", got[1])
	}
	if got[1].Medida != store.MedidaUnidade || got[1].Quantidades[0] != 1 {
		t.Fatalf("UN %#v", got[1])
	}
	if got[2].Nome != "Snack Mucilon Tomate 35g" || got[2].Valor != 5.89 {
		t.Fatalf("related card %#v", got[2])
	}
}

func TestParseSearch_ListPriceMaiorQueAtualEIndicacao(t *testing.T) {
	pagina, err := carrefour.ParseSearch([]byte(`{
		"products":[{"productName":"Cenoura","brand":"Carrefour","items":[{
			"measurementUnit":"kg","unitMultiplier":0.2,
			"sellers":[{"commertialOffer":{"Price":5.49,"ListPrice":7.98}}]
		}]}],
		"recordsFiltered":1,
		"pagination":{"current":{"index":1},"next":{"index":0},"perPage":15}
	}`))
	if err != nil {
		t.Fatal(err)
	}

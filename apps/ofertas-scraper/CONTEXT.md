...
Sistema que observa preços de Produtos em Mercados: pelos PDFs das Fontes (Documento → Extrator) e pelas buscas nos sites (Coleta). Persiste Produtos, Mercados, Marcas e histórico de preços; Artefatos servem ao debug do encarte.

## Language

**Oferta**:
Observação de preço única no catálogo: valor (preço de prateleira / unitário sem condição — o preço “cheio” do bloco), `quantidades[]` (≥1, mesma Medida), `dataInicio` e `dataExpiracao` (vigência; `dataInicio` ≤ `dataExpiracao`; a expiração pode estar no passado para histórico; ambas podem ser futuras), Indicação Promocional, promoção opcional e Comparativo opcional; refere-se a um Produto em um Mercado, com Marca opcional. Unicidade: mesmo Produto + Mercado + valor + `quantidades` + Medida + vigência + promoção + Comparativo + Marca (ausente conta como ausente) → a mesma Oferta; Fonte, dia, Documento, Coleta e Indicação Promocional não distinguem. Quando a extração ou a Coleta reencontra a mesma chave, a origem associa-se à Oferta já existente — não cria clone. Se um Documento reprocessa ou uma Coleta do dia substitui o conjunto e a Oferta fica sem nenhum Documento e sem nenhuma Coleta, ela sai do catálogo. Vários tamanhos no mesmo preço do encarte (lista discreta explícita, ex. 90g / 150g / 200g) são uma só Oferta; preços ou blocos distintos geram Ofertas distintas; intervalo vago ou “diversos tamanhos” sem números não vira Oferta. Domain normaliza `quantidades` (dedup + ordem crescente). Promoção e Comparativo, quando houver, aplicam ao bloco inteiro — o Comparativo não cria segunda Oferta. Preço unitário por tamanho é expansão do consumidor da base, não linhas extras persistidas. Cada data carrega origem (`origemDataInicio` / `origemDataExpiracao`) com valor `extrator`, `filename`, `primeiraDescoberta` (só no início do encarte) ou `coleta`. Cascata de `dataInicio` no encarte: Extrator → início distinto no nome (incl. intervalo no filename; token ISO único não conta como início) → dia da primeira descoberta na Fonte (menor dia de qualquer Documento com aquele nome na Fonte, independente do estado). Cascata de `dataExpiracao` no encarte: Extrator → fim no nome (intervalo ou token ISO; sempre tentada; sem flag na Fonte); se ambas falharem, Falha de Extração — sem inventar fim. As duas cascatas são independentes. Oferta de Coleta: `dataInicio` e `dataExpiracao` são o dia da Coleta; origens `coleta` — sem Coleta naquele dia, não há vigência de vitrine. Toda Oferta tem `dataInicio`, `dataExpiracao` e as duas origens — não há forma legítima sem esses campos. O histórico de preços de um Produto é o conjunto de Ofertas (filtrável); o “período atual” é filtro do consumidor da base, não um estado embutido no Produto. Também pode ser criada, alterada ou apagada fora do Extrator e da Coleta: criação exige ≥1 Documento ou ≥1 Coleta associado; a unicidade continua valendo (origem que reencontra a chave associa-se à Oferta já existente, inclusive se ela nasceu na administração); editar campos da chave que colidam com outra Oferta existente é rejeitado (sem merge automático); apagar remove a Oferta do catálogo e de todas as associações Documento↔Oferta e Coleta↔Oferta.
...
_Avoid_: Promoção (como boolean), flag promoção, emOferta, desconto
